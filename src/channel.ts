...
import { type ChannelSession } from "./channel-sessions";
import {
  capturePane,
  hasSession,
  killSession,
...
function encodeProjectDir(projectDir: string): string {
  // Hans bug fix: Claude Code encodes both `/` AND whitespace as `-` in
  // ~/.claude/projects/<encoded>/. Without the \s replace, dirs with spaces
  // (e.g. "Claude Workspace") get a mismatched path → tail finds nothing.
