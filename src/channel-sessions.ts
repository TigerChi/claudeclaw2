...
// v3 difference vs hans's v2: stored at channel-sessions.json (not sessions.json)
// so v1's per-thread sessions.json can coexist during the transition. v1's
// commands continue to read the legacy sessions.json via src/sessions.ts.
const FILE_PATH = join(".claude", "claudeclaw", "channel-sessions.json");
